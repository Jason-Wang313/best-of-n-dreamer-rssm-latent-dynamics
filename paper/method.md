...
For each planning state we generate a finite pool of action sequences. Each candidate receives latent diagnostics and a score from an RSSM-style model or analytic latent proxy. The selected utility curve for a scorer is computed exactly with the finite selected-tail estimator.
