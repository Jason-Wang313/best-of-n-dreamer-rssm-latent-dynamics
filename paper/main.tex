...
\title{\raggedright When Latent Imagination Lies:\\Best-of-\(N\) Selection Can Amplify\\RSSM World-Model Errors}
...
\begin{abstract}
Latent world models often plan by imagining many futures and selecting the candidate with the largest predicted value. Extra samples then appear to buy better plans almost for free. We study a selected-tail failure mode of this procedure in recurrent state-space model (\rssm)-style latent dynamics: increasing the number of imagined candidates can raise selected imagined value while selected executed utility stagnates or decreases. We first derive an exact finite, tie-aware law for the utility selected by Best-of-\(N\) from an empirical candidate pool. The law separates aligned selection, anti-aligned selection, and the latent-real mismatch regime where the model's score improves in the tail but executed utility does not. We then evaluate controlled hidden-mode dynamics, a small CPU-trained \rssm-style model, receding-horizon planning, out-of-distribution stress grids, and three lightweight Gymnasium toy-text benchmarks. Diagnostics based on uncertainty, posterior-prior disagreement, decoder consistency, and pilot-calibrated value recover substantial selected-tail utility in these settings. The evidence is intentionally scoped: it is controlled, CPU-scale evidence about \rssm-style selected tails, not a full Dreamer benchmark or robotics validation.
\end{abstract}

\section{Introduction}

World models make planning attractive because an agent can evaluate many imagined futures before acting. In latent model-based reinforcement learning, including recurrent state-space model (\rssm) agents such as PlaNet and Dreamer \citep{hafner2019planet,hafner2020dreamer,hafner2021dreamerv2,hafner2023dreamerv3}, this planning step often reduces to a simple rule: sample or optimize a set of candidate action sequences, score them under the learned latent model, and execute the candidate with the largest imagined return. A larger candidate set should improve the selected score. The open question is whether it improves the selected action after the real environment executes it.

This paper studies a narrow but important failure mode. If the high-scoring tail of a latent model is overoptimistic, then Best-of-\(N\) can become a mechanism for finding model errors. The selected imagined value may rise with \(N\), yet the selected executed utility may stagnate or fall. This is not a paradox and not a universal indictment of more planning. It is the expected behavior of selection under score-utility mismatch: selection amplifies whatever relationship holds in the upper tail of the score used for selection.

We make three contributions.
First, we give a finite tie-aware law for the expected utility selected by Best-of-\(N\) from any empirical score/utility pool. Unlike asymptotic order-statistic arguments, the law is exact for the finite candidate pools produced by the experiments and includes score ties.
Second, we instantiate the law in \rssm-style latent imagination, where each candidate has a latent score, architecture-aware diagnostics, and a utility observed only by executing the selected action sequence in hidden-mode dynamics.
Third, we report controlled evidence across analytic hidden-mode rollouts, a small learned \rssm-style model, receding-horizon planning, label-budget calibration, OOD stress regimes, and three toy-text Gymnasium benchmarks. The main finding is bounded: in these CPU-scale settings, raw high-\(N\) latent selection can amplify selected-tail errors, and diagnostic repairs can recover a substantial fraction of the oracle gap without erasing all risk.

\section{Finite Best-of-\(N\) Selection Law}

Consider a finite empirical pool \(\pool=\{(\score_i,\utility_i)\}_{i=1}^m\), where \(\score_i\) is the score used for selection and \(\utility_i\) is the measured utility we care about. In our experiments \(\score_i\) is usually an imagined latent return or a repaired latent score, while \(\utility_i\) is the utility obtained by executing the action sequence in the hidden-mode environment. Best-of-\(N\) draws \(N\) candidates independently with replacement from \(\pool\), selects a draw with maximum score, and breaks score ties uniformly among the tied top-score draws.
...
\begin{proposition}[Finite tie-aware selected utility]
\label{prop:finite-law}
For any \(N\geq 1\), the expected measured utility selected by Best-of-\(N\) is
\begin{equation}
\label{eq:finite-law}
...
The bracketed term is exactly the probability that the maximum sampled score lies in tie group \(g\). Conditional on that event, uniform tie-breaking makes the selected utility equal to the tie group's mean utility. This proof is included in Appendix~\ref{app:proof}. The law makes no assumption that scores are calibrated, monotone, or even useful. If score and measured utility are aligned in the upper tail, Best-of-\(N\) improves utility. If they are anti-aligned, it worsens utility. The failure mode studied here is a latent-real mismatch: the selected latent score \(\Expect[\score_{\mathrm{sel}}(N)]\) increases with \(N\), while \(\Expect[\utility_{\mathrm{sel}}(N)]\) is flat or decreases. Figure~\ref{fig:theory-main} validates the law against Monte Carlo simulation and shows aligned, constant, and anti-aligned regimes.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{figure5_exact_law_validation.png}
  \caption{The finite tie-aware law exactly predicts selected utility curves from empirical candidate pools. Monte Carlo error is below \(0.01\) in this validation. Oracle scoring improves selected utility with \(N\), constant utility remains constant, and anti-aligned scoring degrades utility.}
...
The experiments are designed to answer four questions: Does raw high-\(N\) latent selection expose selected-tail mismatch? Does the failure appear with a learned \rssm-style model, not only an analytic proxy? Can \rssm-specific diagnostics recover utility? Where does the effect disappear or reverse?
...
In receding-horizon planning (Experiment F), raw high-\(N\) selection underperforms both oracle and repair. In the controlled planner, raw \(N=64\) mean return is \(-3.08\), compared with raw \(N=1\) at \(-0.98\), combined repair at \(0.77\), and oracle at \(1.90\). In the learned planner, raw \(N=64\) is \(-1.26\), raw \(N=1\) is \(-0.19\), combined repair is \(0.97\), and oracle is \(2.62\). Experiment G shows that pilot calibration does not require evaluation leakage: with five seeds, 16 labels improve \(N=64\) utility by \(3.31\) over raw, 128 labels by \(3.97\), and 1000 labels by \(4.30\), closing \(57.1\%\) of the raw-to-oracle gap.

\paragraph{OOD regimes and lightweight benchmarks.}
Experiment H sweeps hidden-mode probability, clue strength, and horizon. Raw high-\(N\) selection is harmful in 30 of 36 regimes, neutral in 1, and helpful in 5; combined repair reduces average harm in high-risk regions by \(5.29\). This heterogeneity is important: the claim is not that large \(N\) is intrinsically bad. Experiment I adds exactly three Gymnasium toy-text benchmarks \citep{towers2024gymnasium}. Selected-tail mismatch appears in CliffWalkingSlippery and Taxi but not materially in FrozenLake. Combined repair improves \(N=64\) utility on all three, with large gains on the two mismatch benchmarks (Table~\ref{tab:gym-main}). These are lightweight standard stress tests, not broad RL validation.
...
\left(\frac{r_g^{\min}-1}{m}\right)^N.
\]
Conditional on this event, all sampled candidates with the maximum score are members of the same tie group. Uniform tie-breaking among the top-score sampled candidates gives the group mean measured utility \(\bar{\utility}_g\). Summing over the disjoint maximum-score groups yields Equation~\ref{eq:finite-law}.
...
\begin{figure}[h]
  ...
  \caption{Closed-loop receding-horizon planning in controlled and learned \rssm-style settings. Raw high-\(N\) planning underperforms repair and oracle.}
...
\begin{figure}[h]
  ...
  \caption{OOD stress grid over hidden-mode probability, clue strength, and horizon. Raw high-\(N\) selection is harmful in many regimes but can be neutral or helpful in others.}
...
\begin{itemize}
  \item exact law validation: \path{results/exact_law_validation.json};
  ...
  \item repair, closed-loop, and label-budget studies: \path{results/experiment_e_repairs.json}, \path{results/experiment_f_closed_loop_planning.json}, and \path{results/experiment_g_label_budget_ablation.json};
  \item OOD and Gymnasium studies: \path{results/experiment_h_ood_stress_grid.json} and \path{results/experiment_i_gymnasium_benchmarks.json};
  \item claim and leakage audits: \path{results/claims_status.json} and \path{results/leakage_audit.json}.
\end{itemize}

The strict multiseed audit reports strong evidence for the controlled latent dynamics family over five seeds: mean raw latent delta \(5.41\), mean raw real delta \(-1.98\), and mean repair improvement \(6.20\). The learned \rssm-style family uses three seeds: mean raw latent delta \(0.96\), mean raw real delta \(-1.64\), and mean repair improvement \(5.30\). These multiseed summaries support the qualitative claims while preserving the small-scale scope of the evidence.
